<template>
  <el-card>
    <el-row :gutter="20" class="header">
      <el-col :span="7">
        <el-input
          :placeholder="$t('table.articleplaceholder')"
          cleartable
          v-model="queryForm.query"
        ></el-input>
      </el-col>
      <el-button type="primary" :icon="Search" @click="initGetArticleList">{{
        $t('table.search')
      }}</el-button>
    </el-row>
    <el-table :data="tableData" stripe style="width: 100%">
      <el-table-column
        :width="item.width"
        :prop="item.prop"
        :label="$t(`article.${item.label}`)"
        v-for="(item, index) in options"
        :key="index"
      >
        <template #default="{ row }" v-if="item.prop === 'title'">
          {{ TrimString(row.title, 16) }}
        </template>
        <template #default="{ row }" v-else-if="item.prop === 'content'">
          {{ TrimString(row.content, 32) }}
        </template>
        <template #default="{ row }" v-else-if="item.prop === 'cover'">
          {{ TrimString(row.cover, 32) }}
        </template>
  ...
  <Dialog
    ...
    v-if="dialogVisible"
    @initArticlesList="initGetArticleList"
...
const initGetArticleList = async () => {
  const res = await getArticles(queryForm.value)
  console.log(res)
